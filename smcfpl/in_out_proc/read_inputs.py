...
def Lee_Hoja_planilla(RutaCompleta, NombreHoja, EsParamHid, *args):
    ...
    if EsParamHid:
        ...
        df = pd__read_excel( RutaCompleta, sheetname=NombreHoja, header=[0, 1], usecols=range(len(args) + 1) )  # uno más para leer columns 'Hidrologías' (columns.names)
    else:
        df = pd__read_excel( RutaCompleta, sheetname=NombreHoja, header=0, usecols=range(len(args)) )
